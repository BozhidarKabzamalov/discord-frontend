	...
	return {
		...
		joinVoiceChannel: async (
			...
			try {
				...
				const stream = await webrtcService.joinVoiceChannel(
					channelId,
					userId,
					username,
					enableVideo
				...
				console.error(
					"[VOICE STORE] Failed to join voice channel:",
					error
				);
				throw error;
			}
		...
		leaveVoiceChannel: async (userId: number, username: string) => {
			try {
				...
				// Also call cleanup to ensure everything is stopped
				...
				console.error(
					"[VOICE STORE] Error leaving voice channel:",
					error
				);
