...
export const sendFriendRequest = async (data: SendFriendRequestData): Promise<{ friendRequest: FriendRequest; message: string }> => {
  const response = await axiosInstance.post('/friend-requests/send', data);
  return response.data;
};

// Accept a friend request
export const acceptFriendRequest = async (requestId: number): Promise<{ friendship: any; message: string }> => {
  const response = await axiosInstance.put(`/friend-requests/${requestId}/accept`);
  return response.data;
};

// Reject a friend request
export const rejectFriendRequest = async (requestId: number): Promise<{ message: string }> => {
  const response = await axiosInstance.put(`/friend-requests/${requestId}/reject`);
  return response.data;
};

// Get pending friend requests (received)
export const getPendingFriendRequests = async (): Promise<{ pendingRequests: FriendRequest[] }> => {
  const response = await axiosInstance.get('/friend-requests/pending');
  return response.data;
};

// Get sent friend requests
export const getSentFriendRequests = async (): Promise<{ sentRequests: FriendRequest[] }> => {
...
export const removeFriend = async (friendId: number): Promise<{ message: string }> => {
  const response = await axiosInstance.delete(`/friend-requests/friends/${friendId}`);
  return response.data;
};
