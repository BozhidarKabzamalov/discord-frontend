...
export const sendDirectMessage = async (data: SendDirectMessageData): Promise<{ directMessage: DirectMessage; message: string }> => {
  const response = await axiosInstance.post('/direct-messages/send', data);
  return response.data;
};

// Get conversation with a specific friend
export const getConversation = async (friendId: number): Promise<{ messages: DirectMessage[] }> => {
  const response = await axiosInstance.get(`/direct-messages/conversation/${friendId}`);
  return response.data;
};

// Get all conversations
export const getAllConversations = async (): Promise<{ conversations: DMConversation[] }> => {
  const response = await axiosInstance.get('/direct-messages/conversations');
  return response.data;
};

// Delete a direct message
export const deleteDirectMessage = async (messageId: number): Promise<{ message: string }> => {
  const response = await axiosInstance.delete(`/direct-messages/${messageId}`);
  return response.data;
};
